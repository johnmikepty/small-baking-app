...
export class TransactionsComponent implements OnInit {
  ...
  onSubmit() {
    ...
    call$.subscribe({
      next: (tx) => { this.successTx.set(tx); this.loading.set(false); this.form.reset({ currency: 'USD' }); },
      error: (err) => { this.errorMsg.set(err.error?.detail ?? 'Transaction failed.'); this.loading.set(false); },
    });
  }
}
